...
// ---------------------------------------------------------------------------
// OWNER PANEL — only reachable by an account with is_owner = true (the
...
// ---------------------------------------------------------------------------
export default function OwnerPanel({ accountId, onBack }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [pub, accts, reqs, inactiveIds, cfg] = await Promise.all([
        ...
        auth.listAccountsForOwner(accountId),
        auth.listPendingRequestsForOwner(accountId),
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <h1 style={styles.title}>Owner Panel</h1>
              ...
              {accounts.map((a) => (
                <tr key={a.id}>
                  <td style={styles.td}>
                    ...
                    {a.isOwner && <span style={styles.ownerTag}>owner</span>}
                  </td>
...
const styles = {
  ...
  ownerTag: { marginLeft: 6, fontSize: 10, background: "#111", color: "#fff", borderRadius: 4, padding: "1px 5px" },
